...
<template>
  <!-- <div class="idea-card grid grid-cols-9 pt-2 pb-1 border-t border-borderColor">
    <div v-if="imagesArray && imagesArray.length" class="col-span-3">
      <div
        :style="{
          position: 'relative',
          overflow: 'hidden',
    ...
    <div :class="imagesArray.length ? 'col-span-6' : 'col-span-9'">
      <div class="title-section flex justify-between items-center">
        <div
          @click="viewDetailIdea()"
          class="title text-xl font-bold text-primaryColor hover:text-secondaryColor hover:underline cursor-pointer break-words line-clamp-2"
          v-html="props?.title"
        ></div>
        <div v-if="props.isDraft" class="status-tag bg-gray-600 text-white">Draft</div>
        <div v-else-if="props.isPublish" class="status-tag bg-green-600 text-white">Release</div>
        <div v-else class="status-tag bg-yellow-600 text-white">Pending</div>
      </div>

      <div class="content-section mb-2 border-0">
        <div class="text-gray-600 text-sm line-clamp-3" v-html="props.description"></div>
      </div>

      <div class="footer-section flex justify-between items-center text-[12px]">
        <div v-if="props.isPublish" class="stats flex items-center gap-3 text-[12px]">
          <div class="flex items-center font-medium text-gray-900">
            <span class="font-medium mr-0.5 text-sm">{{ props.totalComment }}</span>
            <i class="fa-regular fa-comment-dots"></i>
          </div>
        ...
        <div class="meta flex items-center space-x-4">
          <div class="flex items-center">
            <i :class="props?.category?.icon + ' fa-solid text-gray-700'"></i>
            <span class="ps-1">{{ props?.category?.title }}</span>
          </div>
          <div class="text-[11px]">{{ props.createdAt }}</div>
          <div class="actions flex space-x-2 ms-5">
            <i
